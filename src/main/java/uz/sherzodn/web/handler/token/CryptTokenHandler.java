 ...
 */
public class CryptTokenHandler implements TokenHandler {
    ...
    public CryptTokenHandler(String secret, Integer sessionTimeoutMinutes) {
    ...
    public SystemUser getUser(String token) {
        ...
        if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
            try {
                ...
                if (validHash) {
                    final SystemUser user = fromJSON(userBytes);
                    if (new Date().getTime() < user.getExpires()) {
                        return user;
                    }
                }
            } catch (IllegalArgumentException e) {
                logger.error("Error during getting user from token ", e);
            }
        }
        return null;
    }
    ...
    public void removeUser(String token) {
        //do nothing since state holded in crypted token
    }
    ...
    private SystemUser fromJSON(final byte[] userBytes) {
        try {
            return new ObjectMapper().readValue(new ByteArrayInputStream(userBytes), SystemUser.class);
        } catch (IOException e) {
